...
import org.openmuc.josistack.internal.acse.asn1.*;
import org.openmuc.josistack.internal.presentation.asn1.*;
...
public final class AcseAssociation {
    ...
    private static final Context_list context_list = new Context_list(
            new byte[]{(byte) 0x23, (byte) 0x30, (byte) 0x0f, (byte) 0x02, (byte) 0x01, (byte) 0x01, (byte) 0x06, (byte) 0x04, (byte)
                    0x52, (byte) 0x01, (byte) 0x00, (byte) 0x01, (byte) 0x30, (byte) 0x04, (byte) 0x06, (byte) 0x02, (byte) 0x51, (byte)
                    0x01, (byte) 0x30, (byte) 0x10, (byte) 0x02, (byte) 0x01, (byte) 0x03, (byte) 0x06, (byte) 0x05, (byte) 0x28, (byte)
                    0xca, (byte) 0x22, (byte) 0x02, (byte) 0x01, (byte) 0x30, (byte) 0x04, (byte) 0x06, (byte) 0x02, (byte) 0x51, (byte)
                    0x01});
    ...
    private static final Result_list presentationResultList = new Result_list(
            new byte[]{(byte) 0x12, (byte) 0x30, (byte) 0x07, (byte) 0x80, (byte) 0x01, (byte) 0x00, (byte) 0x81, (byte) 0x02, (byte)
                    0x51, (byte) 0x01, (byte) 0x30, (byte) 0x07, (byte) 0x80, (byte) 0x01, (byte) 0x00, (byte) 0x81, (byte) 0x02, (byte)
                    0x51, (byte) 0x01});
    ...
    private static final BerObjectIdentifier directReference = new BerObjectIdentifier(new byte[]{(byte) 0x02, (byte) 0x51, (byte) 0x01});
    private static final BerInteger indirectReference = new BerInteger(new byte[]{(byte) 0x01, (byte) 0x03});

    private static final BerObjectIdentifier default_mechanism_name = new BerObjectIdentifier(new byte[]{0x03, 0x52, 0x03, 0x01});
    ...
    public void accept(ByteBuffer payload) throws IOException {
        ...
        Myexternal.SubChoice_encoding encoding = new Myexternal.SubChoice_encoding(new BerAnyNoDecode(payloadLength), null, null);
        ...
        AARE_apdu aare = new AARE_apdu(null, application_context_name, aareAccepted, associateSourceDiagnostic, null, null, null, null,
     ...
     * @throws IOException
     */
    void startAssociation(ByteBuffer payload, InetAddress address, int port, InetAddress localAddr, int localPort, String
            authenticationParameter, byte[] sSelRemote, byte[] sSelLocal, byte[] pSelRemote, ClientTSap tSAP, int[] apTitleCalled, int[]
            apTitleCalling, int aeQualifierCalled, int aeQualifierCalling) throws IOException {
        if (connected == true) {
        ...
        if (authenticationParameter != null) {
            ...
            authentication_value = new Authentication_value(new BerGraphicString(authenticationParameter.getBytes()), null, null);
        }

        AARQ_apdu aarq = new AARQ_apdu(null, application_context_name, called_ap_title, called_ae_qualifier, null, null, calling_ap_title,
                                       calling_ae_qualifier, null, null, sender_acse_requirements, mechanism_name, authentication_value,
                                       null, null, userInformation);
        ...
        CP_type.SubSeq_normal_mode_parameters normalModeParameter = new CP_type.SubSeq_normal_mode_parameters(null, pSelLocalBerOctetString,
                                                                                                              new BerOctetString(
                                                                                                                      pSelRemote),
                                                                                                              context_list, null, null,
                                                                                                              null, userData);
        ...
        ByteBuffer res = null;
        res = startSConnection(ssduList, ssduOffsets, ssduLengths, address, port, localAddr, localPort, tSAP, sSelRemote, sSelLocal);
     ...
     * @throws IOException
     */
    private ByteBuffer startSConnection(List<byte[]> ssduList, List<Integer> ssduOffsets, List<Integer> ssduLengths, InetAddress address,
                                        int port, InetAddress localAddr, int localPort, ClientTSap tSAP, byte[] sSelRemote, byte[]
            sSelLocal) throws IOException {
        if (connected == true) {
            throw new IOException();
        }
        ...
        if (spduType != 0x0e) {
            throw new IOException(
                    "ISO 8327-1 header wrong SPDU type, expected ACCEPT (14), got " + getSPDUTypeString(spduType) + " (" + spduType + ")");
        }
            ...
            switch (parameterType) {
                ...
                case 0x34:
                    ...
                    if (calledSessionSelector != 0x01) {
                        throw new IOException(
                                "SPDU header parameter 'Called Session Selector (52)' is " + calledSessionSelector + ", expected 1");
                    }
                    break;
    ...
    private void sendSessionLayer(List<byte[]> ssduList, List<Integer> ssduOffsets, List<Integer> ssduLengths) throws IOException {
    ...
    void listenForCn(ByteBuffer pduBuffer) throws IOException, TimeoutException {
        ...
        if (spduType != 0x0d) {
            throw new IOException(
                    "ISO 8327-1 header wrong SPDU type, expected CONNECT (13), got " + getSPDUTypeString(spduType) + " (" + spduType + ")");
        }
