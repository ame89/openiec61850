...
final class DataDefinitionResParser {
    ...
    static LogicalNode parseGetDataDefinitionResponse(ConfirmedServiceResponse confirmedServiceResponse, ObjectReference lnRef) throws
            ServiceError {
        ...
        for (StructComponent fcComponent : structure.seqOf) {
            if (fcComponent.componentName == null) {
                throw new ServiceError(ServiceError.PARAMETER_VALUE_INAPPROPRIATE, "Error decoding GetDataDefinitionResponsePdu");
            }
            ...
            if (fcString.equals("LG") || fcString.equals("GO") || fcString.equals("GS") || fcString.equals("MS") || fcString.equals("US")) {
                continue;
            }
    ...
    private static List<FcDataObject> getFcDataObjectsFromSubStructure(ObjectReference lnRef, Fc fc, SubSeqOf_components structure)
            throws ServiceError {
        ...
        for (StructComponent doComp : structComponents) {
            ...
            List<FcModelNode> children = getDoSubModelNodesFromSubStructure(doRef, fc, doComp.componentType.structure.components, false);
            if (fc == Fc.RP) {
    ...
    private static List<FcModelNode> getDoSubModelNodesFromSubStructure(ObjectReference parentRef, Fc fc, SubSeqOf_components structure,
                                                                        boolean parentWasArray) throws ServiceError {
    ...
    private static FcModelNode getModelNodesFromTypeSpecification(ObjectReference ref, Fc fc, TypeSpecification mmsTypeSpec, boolean
            parentWasArray) throws ServiceError {
        ...
        if (mmsTypeSpec.array != null) {
            ...
            for (int i = 0; i < numArrayElements; i++) {
                arrayChildren.add(getModelNodesFromTypeSpecification(new ObjectReference(ref + "(" + Integer.toString(i) + ")"), fc,
                                                                     mmsTypeSpec.array.elementType, true));
            }
        ...
        if (mmsTypeSpec.structure != null) {
            List<FcModelNode> children = getDoSubModelNodesFromSubStructure(ref, fc, mmsTypeSpec.structure.components, parentWasArray);
    ...
    private static BasicDataAttribute convertMmsBasicTypeSpec(ObjectReference ref, Fc fc, TypeSpecification mmsTypeSpec) throws
            ServiceError {
        ...
        if (mmsTypeSpec.bit_string != null) {
            ...
            throw new ServiceError(ServiceError.PARAMETER_VALUE_INAPPROPRIATE, "FLOAT of size: " + floatSize + " is not supported.");
