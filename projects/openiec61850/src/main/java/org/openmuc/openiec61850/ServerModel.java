...
public final class ServerModel extends ModelNode {
     ...
     *
     * @param objectReference the object reference of the node that is being searched for. It has a syntax like "ldname/ln.do....".
     * @param fc              the functional constraint of the requested model node. May be null for Logical Device and Logical Node
     *                        references.
    ...
    FcModelNode getNodeFromVariableDef(VariableDef variableDef) throws ServiceError {
        ...
        if (objectName == null) {
            throw new ServiceError(ServiceError.FAILED_DUE_TO_COMMUNICATIONS_CONSTRAINT, "name in objectName is not selected");
        }

        SubSeq_domain_specific domainSpecific = objectName.domain_specific;

        if (domainSpecific == null) {
            throw new ServiceError(ServiceError.FAILED_DUE_TO_COMMUNICATIONS_CONSTRAINT, "domain_specific in name is not selected");
        }
        ...
        if (index1 == -1) {
            throw new ServiceError(ServiceError.FAILED_DUE_TO_COMMUNICATIONS_CONSTRAINT, "invalid mms item id: " + domainSpecific.itemId);
        }
